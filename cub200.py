...
class CUB200(torch.utils.data.Dataset):
    ...
    def __init__(self, root, train=True, transform=None, target_transform=None,
        ...
        if self._train:
            self._train_data, self._train_labels = pickle.load(open(
                os.path.join(self._root, f'processed/train_{self.resolution}.pkl'), 'rb'))
            assert (len(self._train_data) == 5994
                    and len(self._train_labels) == 5994)
        else:
            self._test_data, self._test_labels = pickle.load(open(
                os.path.join(self._root, f'processed/test_{self.resolution}.pkl'), 'rb'))
    ...
    def _checkIntegrity(self):
        ...
        """
        return (
            os.path.isfile(os.path.join(self._root, f'processed/train_{self.resolution}.pkl'))
            and os.path.isfile(os.path.join(self._root, f'processed/test_{self.resolution}.pkl')))
    ...
    def _extract(self):
        ...
        pickle.dump((train_data, train_labels),
                    open(os.path.join(self._root, f'processed/train_{self.resolution}.pkl'), 'wb'))
        pickle.dump((test_data, test_labels),
                    open(os.path.join(self._root, f'processed/test_{self.resolution}.pkl'), 'wb'))
        # sys.exit()