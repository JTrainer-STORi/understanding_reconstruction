...
class SoftRelu(nn.Module):
    @nn.compact
    def __call__(self, x, use_softplus = False, beta = 1.):
        if use_softplus:
...
class BasicBlock(nn.Module):
    ...
    @nn.compact
    def __call__(self, x, act, train=True, use_softplus = False, beta = 1.):
        """
        Run Basic Block.
        Args:
        ...
        x = ops.batch_norm(x,
                           ...
                           dtype=self.dtype) 
        x = SoftRelu()(x, use_softplus = use_softplus, beta = beta)
